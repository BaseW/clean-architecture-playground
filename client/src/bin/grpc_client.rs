use client::grpc::{create_todo, delete_todo, find_todo, get_todos, update_todo};
...
async fn main() {
    ...
    if args.len() < 2 {
        println!("Usage: grpc_client <command>");
        return;
    }
    ...
    match command.as_str() {
        ...
        "find_todo" => {
            if args.len() < 3 {
                println!("Usage: grpc_client find_todo <id>");
                return;
            }
        ...
        "create_todo" => {
            if args.len() < 3 {
                println!("Usage: grpc_client create_todo <title>");
                return;
            }
        ...
        "update_todo" => {
            if args.len() < 4 {
                println!("Usage: grpc_client update_todo <id> <title>");
                return;
            }
            let id = args[2].parse::<i64>().unwrap();
        ...
        "delete_todo" => {
            if args.len() < 3 {
                println!("Usage: grpc_client delete_todo <id>");
                return;
            }
            let id = args[2].parse::<i64>().unwrap();
